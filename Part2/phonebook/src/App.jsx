  ...
  const addPerson = (e) => {
    ...
    if (checkDuplicateName) {
      ...
      const confirmUpdate = window.confirm(
        `${person.name} is already added to phonebook, replace phone number with new one??`
      );
      ...
      personService
        ...
        .catch((error) => {
          setErrorMessage(`information of ${newName} has been deleted`);
          setTimeout(() => {
            setErrorMessage(null);
          }, 5000);
        });

      setErrorMessage(`Changed ${newName} number to new number: ${newNumber}`);
      setTimeout(() => {
        setErrorMessage(null);
      }, 5000);
      setNewName("");
